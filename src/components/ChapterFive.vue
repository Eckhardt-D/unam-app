<template>
  <div class="hello">
    <div class="container">
      <div v-for="(set, index) in chapterFive" class="card questionBlock my-3 mb-5" :key="set.q">
        <p class="mt-2">{{ index + 1 }}: {{ set.q }}</p>
        <textarea rows="4" @focus="closeDialog(index)" v-model="answer[index]" class="form-control mx-2 my-2 w-75 mx-auto" type="text" name="answer" placeholder="please answer here"></textarea>
...
export default {
  ...
  data () {
    return {
      chapterFive: this.$store.state.chapterFive,
      answer: [],
      check: [],
      rating: [],
      overallRating: 0
